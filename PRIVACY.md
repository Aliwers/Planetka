...
Planetka is designed for local dictation.
...
- Successful transcripts, timing statistics, corrections, and preferences are
  stored under `~/Library/Application Support/Planetka`.
...
Planetka uses the network to download the speech model through
...
macOS Keychain (service `com.local.planetka.ai`), never in preferences
